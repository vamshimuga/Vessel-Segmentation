...
namespace MicrosoftResearch { namespace Cambridge { namespace Sherwood
{
  ...
  private:
    ...
    public:
      ...
      S localParentStatistics_, localLeftChildStatistics_, localRightChildStatistics_;

      std::vector<S> partitionStatistics_;
      std::vector<float> localResponses_;
      std::vector<float> localThresholds;

      Random random_;

      ThreadLocalData(Random& random, ITrainingContext<F,S>& trainingContext_, const TrainingParameters& parameters, IDataPointCollection const & data):random_(random.Next(),random.patchSize)
      {
        localMaxGain = 0.0;
        localBestThreshold = 0.0;
        localParentStatistics_ = trainingContext_.GetStatisticsAggregator();

        localLeftChildStatistics_ = trainingContext_.GetStatisticsAggregator();
        localRightChildStatistics_ = trainingContext_.GetStatisticsAggregator();
        ...
        localThresholds.resize(parameters.NumberOfCandidateThresholdsPerFeature);
        localResponses_.resize(data.Count());
  ...
  public:
    ...
    progress_(progress)
    {
      ...
      for (int threadIndex = 0; threadIndex < maxThreads_; threadIndex++)
        threadLocalData_.push_back( ThreadLocalData(random, trainingContext_, parameters_, data_) );
    }
    ...
    void TrainNodesRecurse(std::vector<Node<F, S> >& nodes, NodeIndex nodeIndex, DataPointIndex i0, DataPointIndex i1, int recurseDepth)
    {
      ...
      {
          threadLocalData_[t].Clear();
          threadLocalData_[t].localParentStatistics_ = parentStatistics_.DeepClone();
      }
      ...
      {
          ThreadLocalData& tl = threadLocalData_[omp_get_thread_num()]; // shorthand
        ...
        for (DataPointIndex i = i0; i < i1; i++)
          tl.localResponses_[i] = feature.GetResponse(data_, indices_[i]);

        int nThresholds;
        if (!parameters_.UseZeroAsThreshold && (nThresholds = ChooseCandidateThresholds(tl.random_, &indices_[0], i0, i1, &tl.localResponses_[0], tl.localThresholds)) == 0)
          continue;
        else if(parameters_.UseZeroAsThreshold)
        {
            nThresholds = 1;
            tl.localThresholds[0] = 0.0f;
        }
        ...
        {
          int b = 0;
          while (b < nThresholds && tl.localResponses_[i] >= tl.localThresholds[b])
            b++;
        ...
        {
          tl.localLeftChildStatistics_.Clear();
          tl.localRightChildStatistics_.Clear();
          for (int p = 0; p < nThresholds + 1 /*i.e. nBins*/; p++)
          {
            if (p <= t)
              tl.localLeftChildStatistics_.Aggregate(tl.partitionStatistics_[p]);
            else
              tl.localRightChildStatistics_.Aggregate(tl.partitionStatistics_[p]);
          }

          // Compute gain over sample partitions
          double gain = trainingContext_.ComputeInformationGain(tl.localParentStatistics_, tl.localLeftChildStatistics_, tl.localRightChildStatistics_);

          if (gain >= tl.localMaxGain)
          {
            tl.localMaxGain = gain;
            tl.localBestFeature = feature;
            tl.localBestThreshold = tl.localThresholds[t];
          }
        }
      }
      ...
      for (int threadIndex = 0; threadIndex < maxThreads_; threadIndex++)
      {
        ...
        if (tl.localMaxGain > maxGain)
        {
          maxGain = tl.localMaxGain;
          bestFeature = tl.localBestFeature;
          bestThreshold = tl.localBestThreshold;
        }
      }
  ...
  public:
      ...
      ProgressStream* progress=0)
    {
      ...
      {
        (*progress)[Interest] << "\rTraining tree "<< t << "..." << std::flush;

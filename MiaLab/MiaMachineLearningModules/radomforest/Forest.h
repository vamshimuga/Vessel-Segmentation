...
namespace MicrosoftResearch { namespace Cambridge { namespace Sherwood
{
  ...
  public:
    ...
    /// <summary>
    /// Creat a copy of self.
    /// </summary>
    /// <returns></returns>
    std::auto_ptr<Forest<F, S> > Clone()
    {
      std::auto_ptr<Forest<F, S> > newforest = std::auto_ptr<Forest<F, S> >(new Forest<F,S>());

      for(int t=0; t<trees_.size(); t++)
      {
          std::auto_ptr<Tree<F,S> > tree = trees_[t]->Clone();
          newforest->trees_.push_back(tree.get());
          tree.release();
      }
